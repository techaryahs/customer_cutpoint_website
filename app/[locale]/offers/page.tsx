...
export default function OffersDiscoveryPage() {
  ...
  const loc = typeof window !== 'undefined' ? localStorage.getItem('glowbiz_location') || 'Mumbai' : 'Mumbai';

  const fetchOffers = async () => {
    setLoading(true);
    try {
      const apiPath = `${BACKEND_URL}${BACKEND_URL.endsWith('/api') ? '' : '/api'}/offers`;
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-taupe/40">{t('loading')}</p>
      </div>
    );
  }
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div className="max-w-xl space-y-4">
                <div className="inline-flex items-center gap-2 bg-gold/20 px-4 py-1.5 rounded-full border border-gold/30">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-goldDark">{t('exclusive')}</span>
                </div>
                <h1 className="text-4xl md:text-6xl font-serif font-bold text-sand tracking-tight leading-tight">
                  {t.rich('title', {
                    flashDeals: (chunks) => <span className="text-goldDark">{chunks}</span>
                  })}
                </h1>
              ...
              <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-sand/40 uppercase tracking-widest">{t('active_region')}</p>
        ...
        <section className="mb-12 flex flex-col lg:flex-row gap-6 items-center justify-between bg-[#FAF9F6] p-4 rounded-[2rem] border border-borderSoft shadow-soft transition-all duration-300">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 no-scrollbar">
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-borderSoft shadow-sm shrink-0">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-taupe/60">{t('validity')}</span>
            </div>
            {[
              { label: t('all'), value: 'all' },
              { label: t('next_2_days'), value: 2 },
              { label: t('this_week'), value: 7 },
              { label: t('this_month'), value: 30 },
            ].map((btn) => (
              <button
                key={btn.value}
        ...
        <AnimatePresence mode="wait">
          {filteredOffers.length > 0 ? (
            ...
            >
              ...
              <h3 className="text-3xl font-serif font-bold text-cocoa mb-4">{t('empty_title')}</h3>
              <p className="text-taupe font-medium max-w-sm mb-10 opacity-60">
                {t('empty_desc', { location: loc })}
              </p>
              <button
      ...
      <div className="relative h-48">
        ...
        <div className="absolute top-4 left-4">
          <div className="bg-red-600 text-white px-4 py-2 rounded-xl shadow-lg border border-red-500/20">
            <p className="text-xs font-black uppercase tracking-widest leading-none mb-1">{t('save')}</p>
          ...
          <div className="bg-white/90 backdrop-blur-md px-4 py-2.5 rounded-2xl flex items-center justify-between shadow-soft border border-white/20">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-cocoa">
                {diffDays <= 0 ? t('expiring_today') : t('days_left', { count: diffDays })}
              </span>
            </div>
            <div className="h-4 w-[1px] bg-cocoa/10" />
            <span className="text-[10px] font-black text-taupe/40 uppercase tracking-widest">{t('until', { date: offer.validUntil })}</span>
          </div>
        </div>
      </div>
    ...
    {(offer.serviceNames?.length ? offer.serviceNames : ['All Services']).map(
      (name: string, i: number) => (
        ...
        <div className="pt-8 mt-4 border-t border-linen flex items-center justify-between gap-4">
          <div>
            <p className="text-[8px] font-black text-taupe/40 uppercase tracking-[0.2em] mb-0.5">{t('location')}</p>
