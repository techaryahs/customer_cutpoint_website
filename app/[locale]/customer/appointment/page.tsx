...
export default function AppointmentsPage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-8 h-8 border-2 border-goldDark border-t-transparent rounded-full mb-3" />
      <p className="text-taupe text-sm font-medium">{t('loading')}</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-transparent pb-24">
      <div className="max-w-6xl mx-auto px-4 pt-10 pb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="space-y-1">
            <h1 className="text-3xl md:text-4xl font-serif font-bold text-cocoa tracking-tight">{t('title')}</h1>
            <p className="text-taupe text-sm font-medium opacity-70">
              {t('active_bookings', { count: bookings.filter(b => b.status === 'booked').length })}
            </p>
          </div>
          <Link href="/search" className="bg-cocoa text-white px-8 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-goldDark transition-all shadow-xl shadow-cocoa/10 flex items-center gap-2 w-fit">
            <span>+</span> {t('book_new')}
          </Link>
        </div>
            ...
            {filteredBookings.length > 0 ? (
              filteredBookings.map((b) => (
                <AppointmentCard 
                  key={b.appointmentId} 
                  booking={b} 
                  status={getApptStatus(b)} 
                  canModify={b.status === 'booked' && (() => { 
                    let dStr = b.date; 
                    if (dStr.includes('-') && dStr.split('-')[0].length === 2) dStr = dStr.split('-').reverse().join('-'); 
                    return (new Date(dStr + 'T' + b.startTime).getTime() - Date.now()) / 60000 > 60; 
                  })()} 
                  onReschedule={(appt) => { 
                    setShowRescheduleModal(appt); 
                    const today = new Date().toISOString().split('T')[0]; 
                    setRescheduleDate(today); 
                    fetchSlots(today, appt); 
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 text-center bg-white rounded-[3.5rem] border border-borderSoft shadow-soft">
                ...
                <p className="text-taupe text-sm max-w-xs mx-auto mb-10 font-medium opacity-60">{t('empty_desc')}</p>
      ...
      <CancelModal 
        ...
        placeName={showCancelModal?.place.name || ""} 
      />
      <RescheduleModal 
        ...
        isSubmitting={isSubmitting} 
        placeName={showRescheduleModal?.place.name || ""} 
        rescheduleDate={rescheduleDate} 
        setRescheduleDate={(d) => { setRescheduleDate(d); if (showRescheduleModal) fetchSlots(d, showRescheduleModal); }} 
